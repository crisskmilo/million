namespace Million.Infra.Data.Repositories.Transversal
{
    ...
    /// <seealso cref="Million.Domain.Interfaces.Repositories.IQueryRepository{T}" />
    public class QueryRepository<T>: IQueryRepository<T> where T: class
    {
        ...
        public virtual T GetObjectById(T myObject)
        {
            ...
            var fieldId = type.GetProperties().FirstOrDefault(prop => Attribute.IsDefined(prop, typeof(KeyAttribute)));
            string sql = string.Concat(" SELECT * FROM ", tableAttr.Name, " WHERE ", fieldId.Name, " = '", GetValueField(fieldId, myObject),"'");
            using (var db = this.DbFactory.GetConnection())
            {
